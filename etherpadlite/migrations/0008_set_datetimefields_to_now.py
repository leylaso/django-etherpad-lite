...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for pad in orm['etherpadlite.Pad'].objects.all():
            pad.created_at = datetime.now()
            pad.updated_at = datetime.now()
            pad.save()

        for padgroup in orm['etherpadlite.PadGroup'].objects.all():
            padgroup.created_at = datetime.now()
            padgroup.updated_at = datetime.now()
            padgroup.save()

        for padsuthor in orm['etherpadlite.PadAuthor'].objects.all():
            padsuthor.created_at = datetime.now()
            padsuthor.updated_at = datetime.now()
            padsuthor.save()

        for padserver in orm['etherpadlite.PadServer'].objects.all():
            padserver.created_at = datetime.now()
            padserver.updated_at = datetime.now()
            padserver.save()
