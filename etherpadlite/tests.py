...
class PadServerTestCase(unittest.TestCase):
  ...
  def setUp(self):
    self.server = PadServer.objects.create(title=TS['title'], url=TS['url'], apikey=TS['apikey'])
...
class PadGroupTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.padGroup = PadGroup.objects.create(group=self.group, server=self.server)
...
class PadAuthorTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.padGroup = PadGroup.objects.create(group=self.group, server=self.server)
    self.author = PadAuthor.objects.create(user=self.user, server=self.server)
...
class PadTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.padGroup = PadGroup.objects.create(group=self.group, server=self.server)
    self.author = PadAuthor.objects.create(user=self.user, server=self.server)
    self.pad = Pad.objects.create(name='foo', server=self.server, group=self.padGroup)
